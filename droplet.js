        ...
        function(
            ...
            dropletConfigText
                ...
                function main(options, imports, register) {
                    ...
                    // Serialize
                    function(session) {
                        // Find the associated droplet Editor
                        ...
                        if (tab != null)( {
                            // From that, find the associated droplet session and
                            // get its floating blocks in serialized form
                            var editor = tab.editor.ace._dropletEditor;
                            return editor.sessions.get(session).floatingBlocks.map(function(block) {
                                return {
                                    block: block.block.stringify(),
                        ...
                        // If there is no associated droplet Editor but
                        // we persisted something from last time, persist it again
                        else if (session.on._droplet_floatingBlocks) {
                            return session.on._droplet_floatingBlocks;
                        }

                        // Otherwise, there can't be any floating blocks.
                        else {
                            return null;
                        }
                    },
                    ...
                    function(state, session) {
                        // We store this data in a hack variable put onto
                        // the "on" function, which was one of the few places
                        // that is not frozen in the Ace session.
...
>>>>>>> b9b5ebb736b6b0f6ee134199092caf5601f87662
                    ...
                    function attachToAce(aceEditor) {
                        ...
                        if (!aceEditor._dropletEditor) {
                            ...
                            // If there were some saved floating blocks, load them back
                            // up again
                            ...
                            aceEditor.on("changeSession", function(e) {
                                ...
                                if (!aceEditor._dropletEditor.hasSessionFor(e.session)) {
                                    ...
                                    if (option != null) {
                                        ...
                                        // If there were some saved floating blocks,
                                        // load them back up.
